    ...
    for row in rows:
        ...
        if isinstance(meta, dict):
            item.update({
                key: meta.get(key)
                for key in (
                    "peso_fundamental", "macro_impact",
                    "macro_score_adjustment", "score_contextual",
                )
                if key in meta
            })
        items.append(item)
