...
Centraliza, de forma auditável, TODAS as análises aplicadas ao longo do
pipeline de seleção de empresas e construção de portfólio (aba Empresas B3).
Cada técnica é vinculada ao estudo/autor que a fundamenta, para reforçar a
credibilidade dos tickers selecionados e dar transparência metodológica.

Usado pela UI para renderizar a seção "Metodologia e referências científicas"
ao final da aba de criação de portfólio.
"""
...
class Metodo:
    """Uma técnica aplicada, com o que faz e a referência que a fundamenta."""
    etapa:      str
    nome:       str
    o_que_faz:  str
...
CATALOGO: list[Metodo] = [
    ...
    Metodo(
        ...
        "O score usa fundamentos consolidados do último ano COMPLETO para "
        "decidir o ano corrente — evita dado parcial e look-ahead.",
        "boa prática contábil", "—",
    ),
    Metodo(
        ETAPA_DADOS, "Exclusão de empresas descontinuadas",
        "Empresas que saíram da B3 são removidas do universo — só ativos "
        "investíveis hoje entram na seleção.",
        "controle de survivorship", "brown1992",
    ),
    Metodo(
        ETAPA_DADOS, "Filtro de liquidez de negociação",
        "Exclui ações com baixo volume diário (mediana de Close×Volume), "
        "onde o spread bid-ask encarece a operação.",
        "microestrutura de mercado", "amihud2002",
    ),
    Metodo(
        ETAPA_DADOS, "Winsorização de outliers",
        "Limita valores extremos aos percentis 5–95 antes do ranking, "
        "evitando que um número absurdo distorça o score do grupo.",
        "Tukey (1962)", "tukey1962",
    ),
    Metodo(
        ETAPA_DADOS, "Imputação MICE",
        "Estima indicadores ausentes usando a correlação entre os demais "
        "(IterativeImputer/BayesianRidge), em vez de mediana cega ou 0,5 fixo.",
        "van Buuren & Groothuis-Oudshoorn (2011)", "vanbuuren2011",
    ),
    ...
    Metodo(
        ...
        "padrão de cross-sectional ranking", "—",
    ),
    Metodo(
        ETAPA_SCORE, "Pesos setoriais calibrados",
        "Cada setor pondera os indicadores que mais importam para ele "
        "(ex.: DY pesa mais em utilities; ROIC em indústria).",
        "Fama & French (1992)", "famafrench1992",
    ),
    Metodo(
        ETAPA_SCORE, "Pesos empíricos Fama-MacBeth (opcional)",
        "Regressão cross-sectional estima empiricamente o quanto cada "
        "indicador prevê qualidade futura, calibrando os pesos.",
        "Fama & MacBeth (1973)", "famamacbeth1973",
    ),
    Metodo(
        ETAPA_SCORE, "Penalidade de instabilidade (CV)",
        "Desconta empresas com resultados voláteis no histórico (coeficiente "
        "de variação) — premia consistência, essencial para dividendos.",
        "qualidade do lucro (earnings quality)", "—",
    ),
    Metodo(
        ETAPA_SCORE, "Penalidade de crowding",
        "Reduz o score de empresas no bucket de valuation mais lotado do "
        "setor, evitando concentração no consenso.",
    ...
    Metodo(
        ...
        "Avalia risco de sobrevivência (alavancagem crítica + margens "
        "negativas) separado do score de rentabilidade.",
        "Altman (1968); Altman & Hotchkiss (2006)", "altman1968",
    ),
    Metodo(
        ETAPA_RESILIENCIA, "Preço justo — Graham",
        "Margem de segurança via √(22,5·LPA·VPA): a ação está cara ou barata "
        "em termos absolutos, não só relativa aos pares.",
        "Graham (1949); Graham & Dodd (1934)", "graham1949",
    ),
    Metodo(
        ETAPA_RESILIENCIA, "Preço justo — Bazin",
        "Preço-teto pelo dividendo: preço justo = dividendo/yield-alvo. "
        "Foco no investidor de renda.",
        "Bazin (1991)", "bazin1991",
    ),
    Metodo(
        ETAPA_RESILIENCIA, "Sustentabilidade de dividendos",
        "Verifica se o dividendo é pagável de forma recorrente (payout + "
        "cobertura por Fluxo de Caixa Operacional), detectando dividend traps.",
        "Damodaran (2012)", "damodaran2012",
    ),

    # 4 — Risco e construção do portfólio
    Metodo(
        ETAPA_RISCO, "Otimização de Markowitz",
        "Calcula pesos que minimizam a variância da carteira sujeitos a teto "
        "por ativo — considera correlações, não só o score.",
        "Markowitz (1952)", "markowitz1952",
    ),
    Metodo(
        ETAPA_RISCO, "Covariância Ledoit-Wolf",
        "Shrinkage da matriz de covariância, estável em janelas curtas — "
        "evita pesos extremos do estimador amostral puro.",
        "Ledoit & Wolf (2004)", "ledoitwolf2004",
    ),
    Metodo(
        ...
        "Fama & French (1993)", "famafrench1993",
    ),
    Metodo(
        ETAPA_RISCO, "Covariância dinâmica DCC-GARCH",
        "Correlações condicionais que variam no tempo e sobem em crises — "
        "captura o risco de cauda que a correlação estática ignora.",
        "Engle (2002); Bollerslev (1986)", "engle2002",
    ),
    Metodo(
        ...
        "Black & Litterman (1992); Idzorek (2005)", "blacklitterman1992",
    ),
    Metodo(
        ETAPA_RISCO, "Diversificação setorial",
        "Limita quantas ações do mesmo setor entram na carteira final, "
        "evitando concentração em um único segmento.",
        "diversificação (Markowitz 1952)", "markowitz1952",
    ),
    Metodo(
        ETAPA_RISCO, "Custos de transação",
        "Backtest desconta corretagem, spread e IR sobre lucro acima da "
        "isenção mensal — retorno realista para PF no Brasil.",
        "modelo de custos PF-BR", "—",
    ),
    ...
    Metodo(
        ...
        "López de Prado (2018)", "lopezdeprado2018",
    ),
    Metodo(
        ETAPA_VALIDACAO, "Teste de estresse histórico",
        "Reaplica choques de crises reais (Subprime, COVID, eleições) à "
        "carteira para estimar perda potencial.",
        "stress testing", "—",
    ),
    Metodo(
        ETAPA_VALIDACAO, "Atribuição Brinson",
        "Decompõe o retorno em efeito de alocação setorial vs. seleção de "
        "ativos.",
        "Brinson, Hood & Beebower (1986)", "brinson1986",
    ),
    Metodo(
        ...
        "Shapley (1953); Lundberg & Lee (2017)", "shapley1953",
    ),
    Metodo(
        ETAPA_VALIDACAO, "Bootstrap (intervalo de confiança)",
        "Reamostra o universo para estimar a incerteza do score — quão "
        "robusto é o ranking a pequenas variações nos dados.",
        "Efron (1979)", "efron1979",
    ),
]
...
def catalogo_por_etapa() -> dict[str, list[Metodo]]:
    """Retorna o catálogo agrupado por etapa, na ordem de exibição."""
...
def referencias_ordenadas() -> list[str]:
    """Retorna a lista de referências bibliográficas em ordem alfabética."""
    return sorted(BIBLIOGRAFIA.values())


def total_metodos() -> int:
    """Número total de técnicas catalogadas."""
    return len(CATALOGO)
