...
def montar_painel(*, agora: dt.datetime | None = None) -> P.Painel:
    ...
    if coletado is None:
        frescor.append(qz.Frescor("Notícias", atualizado_em=None,
                                  disponivel=False, erro=MSG_SEM_COLETA))
    else:
...
def render_atualizacao(pn: P.Painel) -> None:
    ...
    ui.barra_frescor(pn)
    if st.button("🔄 Atualizar notícias agora", type="primary",
                 help="Consulta os provedores configurados. Respeita o "
                      "orçamento de requisições de cada um."):
        ...
        with st.spinner("Consultando os provedores de notícias..."):
            st.session_state[CHAVE_COLETA] = coletar_noticias(tickers)
        st.rerun()
